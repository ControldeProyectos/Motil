...
import type { Suministro, EstadoSuministro } from '../../types';
...
const estBadge = (est: EstadoSuministro): 'ok' | 'info' | 'warn' | 'neutral' =>
  est === 'ENTREGADO' ? 'ok' : est === 'EN_TRANSITO' ? 'info' : est === 'EN_ADUANA' ? 'warn' : 'neutral';

export function SuministrosDemo({ suministros, alerts, onAdd, onDelete }: Props) {
  ...
  const handleAdd = () => {
    ...
    if (!result.success) {
      const fe: Partial<Record<keyof SuministroInput, string>> = {};
      for (const issue of result.error.issues) fe[issue.path[0] as keyof SuministroInput] = issue.message;
      setErrors(fe);
      return;
    }
    onAdd({
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        ...
        <button onClick={() => setOpen(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '7px 14px', borderRadius: 7, background: '#2e6da4', color: 'white', fontSize: 11, fontFamily: 'Inter', fontWeight: 600, cursor: 'pointer', border: 'none' }}>
          + Nuevo Suministro
        </button>
      </div>
      ...
      {alerts.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          {alerts.map(s => (
            <div key={s.id} style={{ background: '#fdecea', border: '1px solid #f5c6c2', borderLeft: '4px solid #c0392b', borderRadius: 8, padding: '11px 14px', marginBottom: 9, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
              <div style={{ fontSize: 18, flexShrink: 0 }}>⚠️</div>
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#c0392b', marginBottom: 2 }}>ALERTA — {s.descripcion}</div>
                <div style={{ fontSize: 11, color: '#4a6080' }}>Proveedor: <strong>{s.proveedor}</strong> · Llegada estimada: {formatDate(s.fechaLlegada)}</div>
                <div style={{ fontSize: 10, color: '#b36a00', marginTop: 3, fontWeight: 600 }}>{alertaLabel(s)} · Necesario: {formatDate(s.fechaNecesaria)}</div>
              </div>
            </div>
          ))}
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            ...
            <tbody>
              {suministros.length === 0 ? (
                <tr><td colSpan={8} style={{ textAlign: 'center', color: '#4a6080', padding: 28, background: '#f0f4f8' }}>Sin suministros registrados.</td></tr>
              ) : suministros.map((s, i) => {
                ...
                return (
                  <tr key={s.id} style={{ borderBottom: '1px solid #dde5ef', background: isAlert ? '#fdecea' : undefined }}>
                    ...
                    <td style={{ padding: '9px 10px', fontFamily: 'DM Mono,monospace', color: late ? '#c0392b' : '#1a2b4a', fontWeight: late ? 700 : 400 }}>{formatDate(s.fechaLlegada)}</td>
                    <td style={{ padding: '9px 10px' }}><Badge variant={estBadge(s.estado)}>{ESTADO_SUMINISTRO_LABEL[s.estado]}</Badge></td>
                    <td style={{ padding: '9px 10px' }}>{isAlert ? <Badge variant="danger">⚠ ALERTA</Badge> : <Badge variant="ok">✓ OK</Badge>}</td>
                    <td style={{ padding: '9px 10px' }}>
                      <button onClick={() => { if (confirm('¿Eliminar?')) onDelete(s.id); }} style={{ background: '#fdecea', color: '#c0392b', border: '1px solid #f5c6c2', fontSize: 10, padding: '3px 9px', borderRadius: 5, cursor: 'pointer', fontWeight: 600 }}>Eliminar</button>
                    </td>
                  </tr>
                );
              })}
