...
import type { Restriccion, EstadoRestriccion } from '../../types';
...
interface Props {
  ...
  onAdd: (r: Restriccion) => void;
  onDelete: (id: string) => void;
  onUpdateEstado: (id: string, estado: EstadoRestriccion) => void;
}
...
export function RestriccionesDemo({ restricciones, onAdd, onDelete, onUpdateEstado }: Props) {
  ...
  const handleAdd = () => {
    ...
    if (!result.success) {
      const fe: Partial<Record<keyof RestriccionInput, string>> = {};
      for (const issue of result.error.issues) fe[issue.path[0] as keyof RestriccionInput] = issue.message;
      setErrors(fe);
      return;
    }
    onAdd({
          ...
          <div key={r.id} style={{ background: 'white', border: '1px solid #dde5ef', borderRadius: 8, padding: 13, marginBottom: 9, borderLeft: `4px solid ${borderColor}`, boxShadow: '0 1px 3px rgba(26,43,74,.05)', opacity: r.estado === 'CERRADA' ? .8 : 1 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 6 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                ...
                <button onClick={() => { if (confirm('¿Eliminar?')) onDelete(r.id); }} style={{ background: '#fdecea', color: '#c0392b', border: '1px solid #f5c6c2', fontSize: 10, padding: '3px 9px', borderRadius: 5, cursor: 'pointer', fontWeight: 600 }}>✕</button>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 14, marginTop: 6, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: 10.5, color: '#4a6080' }}>📅 Fecha atención: <span style={{ color: '#1a2b4a', fontWeight: 500 }}>{r.fechaAtencion ? formatDate(r.fechaAtencion) : '—'}</span></span>
            </div>
            {r.obs && (
              <div style={{ fontSize: 10.5, color: '#4a6080', marginTop: 7, background: '#f0f4f8', padding: '6px 8px', borderRadius: 5, borderLeft: '2px solid #c5d4e4' }}>📝 {r.obs}</div>
            )}
            {r.estado !== 'CERRADA' && (
              <div style={{ display: 'flex', gap: 7, marginTop: 10 }}>
                {r.estado !== 'EN_PROCESO' && (
                  <button onClick={() => onUpdateEstado(r.id, 'EN_PROCESO')} style={{ padding: '5px 12px', borderRadius: 7, border: '1.5px solid #dde5ef', background: '#f0f4f8', color: '#1a2b4a', fontFamily: 'Inter', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                    Marcar en proceso
                  </button>
                )}
                <button onClick={() => onUpdateEstado(r.id, 'CERRADA')} style={{ padding: '5px 12px', borderRadius: 7, border: 'none', background: '#2e6da4', color: 'white', fontFamily: 'Inter', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                  ✓ Cerrar restricción
                </button>
              </div>
            )}
